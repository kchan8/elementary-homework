...
//	fileUpload is service (controller -> service)
	function workbookUploadCtrl ($routeParams, $scope, fileUpload, authentication, $location){
		...
		$scope.upload = function(){
			...
			} else if ($scope.workbook.files !== undefined
					&& checkFilename($scope.workbook.files, $scope.workbook.pages)){
				if ($scope.workbook.pages !== ($scope.workbook.files.length - 1)){
					$scope.formError = "No. of pages mismatch with the chosen files.";
				} else {
					var fsize = 0;
					for (var i=0; i<$scope.workbook.files.length; i++){
						fsize += $scope.workbook.files[i].size;
					}
					fsize = fsize / 1024 / 1024;
					if (fsize > 15){
						$scope.formError = "Upload files size exceed 15 MB limit.";
					} else {
						uploadUrl = "/api/workbook/upload";
