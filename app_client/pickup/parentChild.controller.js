...
//		console.log(strTime)
	...
	function parentChildCtrl($routeParams, $scope, $uibModal, $window, $location, $localStorage, $sessionStorage, authentication){
		var vm = this;
		...
		// get parentID and the date range, send to api
		// and get back pickup data, time and driver info
		// allow to cancel but not able to add
		if ($routeParams.parentID !== undefined &&
			$routeParams.from !== undefined &&
			$routeParams.to !== undefined){			
			vm.parentID = $routeParams.parentID;
			vm.dateFrom = str2Date($routeParams.from);
			vm.dateTo = str2Date($routeParams.to);
		} else {			
...
//				$scope.weekDate.setDate($scope.startDate.getDate() + j)
			...
			authentication.getStudentPickup(vm.client, vm.parentID, $scope.startDate, $scope.endDate)
			.then(function(res){
				$scope.students = [];
				var pickups = res.pickups;
				res.students.forEach(function(student){
					// get time as pickup and driver
					var school;
					var pickup = []
					res.pickups.sort(function(a, b){
						return new Date(a.date) - new Date(b.date);
					})
					for(var i=0; i<res.pickups.length; i++){
						if (i == 0){
							school = res.pickups[i].school;
						}
						if (res.pickups[i].studentID.startsWith(student)){
							pickup.push({pickup: res.pickups[i].timeStr,
										 driver: res.pickups[i].driverID
								})
						}
					}
					vm.drivers = res.drivers;
					if (pickup.length != 0){						
						$scope.students.push({firstName: student,
							school: school,
							pickup: pickup})
					}
				})
		...
		setDates(vm.dateFrom, vm.dateTo);
	}

	
	angular
	.module('asgApp')
	.controller('parentChildCtrl', parentChildCtrl)
